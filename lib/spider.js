...
class Spider {
	constructor(config) {
		...
		if (config.proxyFile) {
			if (config.debug) {
				...
				self.logger.info(`PROXY ENV : [ -- ${config.proxyFile} -- ]`);

				let proxy = utils.getProxy('../data/' + config.proxyFile);
				self.crawler.on('schedule', configions => proxy.setOptProxy(configions));
			}
		}
	...
	toCSV(file, data) {
		...
		if (self.doneSet.has(data[self.doneSetIndex])) {
			self.logger.info('skipping dumlicate ' + self.header[self.doneSetIndex] + ':' + data[self.doneSetIndex]);
			self.logger.warn('dumlicated data detected');
			self.logger.warn('skipping dumlicated data:' + data.join());
		} else {
			self.logger.verbose('Spider saving data : ' + data.toString());
